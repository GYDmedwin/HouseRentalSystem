...
public class OrderDao {
	...
	public boolean add(OrderBean order) {
		try {
			String sql = "Insert into order_rent(household,tenant_num,house_num,deal,charge,tax) values(?,?,?,?,?,?)";
			Object[] params = {order.household,order.tenant_num,order.house_num,order.deal,order.charge,order.tax};
	...
	public List<OrderBean> getOrderList_H() {
		try {
			String sql = "SELECT\n" +
					...
					"INNER JOIN tenant ON (tenant.tenant_id =order_rent.tenant_num) where order_rent.household=?;";
	...
	public List<OrderBean> getOrderList_T() {
		try {
			String sql = "SELECT\n" +
					...
					"Order_rent\n" +
					"INNER JOIN house ON (house.house_id = order_rent.house_num)\n" +
					"INNER JOIN householder ON (householder.householder_id =order_rent.household) where order_rent.tenant_num=?;";
